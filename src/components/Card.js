...
import {getTxPending} from '../utils/index'
...
export default forwardRef(function Card(props, ref) {
  ...
  function copyBuyer(buyer) {
    
    navigator.clipboard.writeText(buyer);
  }

  function inforTx() {
    const tx= getTxPending()[0];
                ...
                histories.map((item, index) => {
                  return (
                    <Box display="flex" justifyContent="space-between">
                      ...
                      <MI.LinkSharp  onClick={() => inforTx(item.buyer)} fontSize="big" style={{ fill: '#c23a3a', cursor: "pointer"}} />
                     </Box>
                  )
                })
              ) : (
